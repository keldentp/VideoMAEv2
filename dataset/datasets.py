...
class NumpyClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            sample = self.dataset_samples[index]
            # T H W C
            buffer = self.load_numpy(sample, sample_rate_scale=scale_t)
            if len(buffer) == 0:
                while len(buffer) == 0:
                    warnings.warn(
                        "video {} not correctly loaded during training".format(
    ...
    def load_numpy(self, sample, sample_rate_scale=1):
        fname = sample

        try:
            vr = self.numpy_loader(fname)
        except Exception as e:
            print(f"Failed to load video from {fname} with error {e}!")
            return []

        length = vr.shape[0] # C x H x W 
